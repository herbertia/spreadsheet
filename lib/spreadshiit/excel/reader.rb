require 'spreadshiit/encodings'
require 'spreadshiit/font'
require 'spreadshiit/formula'
require 'spreadshiit/link'
require 'spreadshiit/note'
require 'spreadshiit/noteObject'
require 'spreadshiit/excel/error'
require 'spreadshiit/excel/internals'
require 'spreadshiit/excel/sst_entry'
require 'spreadshiit/excel/worksheet'
...
class Reader
  include Spreadshiit::Encodings
  include Spreadshiit::Excel::Internals
  ROW_BLOCK_OPS = {
  ...
  def encoding codepage_id
    name = CODEPAGES.fetch(codepage_id) do
      raise Spreadshiit::Errors::UnknownCodepage, "Unknown Codepage 0x%04x" % codepage_id
    end
    ...
    if RUBY_VERSION >= '1.9'
      begin
        ...
        raise Spreadshiit::Errors::UnsupportedEncoding, "Unsupported encoding with name '#{name}'"
      end
    else
      name
    end
  ...
  def extend_internals version
    require 'spreadshiit/excel/internals/biff%i' % version
  ...
  def extend_reader version
    require 'spreadshiit/excel/reader/biff%i' % version
