  ...
  module Encodings
    if RUBY_VERSION >= '1.9'
      def client string, internal='UTF-16LE'
        ...
        string.encode Spreadshiit.client_encoding
      end
      def internal string, client=Spreadshiit.client_encoding
        string = string.dup
        string.force_encoding client
        string.encode('UTF-16LE').force_encoding('ASCII-8BIT')
      end
      def utf8 string, client=Spreadshiit.client_encoding
        string = string.dup
        string.force_encoding client
        string.encode('UTF-8')
      end
      ...
      def build_output_encoding(to_encoding)
        [to_encoding, Spreadshiit.enc_translit, Spreadshiit.enc_ignore].compact.join('//')
      end

      def client string, internal='UTF-16LE'
        string = string.dup
        key = [Spreadshiit.client_encoding, internal]
        iconv = @@iconvs[key] ||= Iconv.new(Spreadshiit.client_encoding, internal)
        iconv.iconv string
      end
      def internal string, client=Spreadshiit.client_encoding, to_encoding = 'UTF-16LE'
        string = string.dup
        key = [to_encoding, client]
        iconv = @@iconvs[key] ||= Iconv.new(build_output_encoding(to_encoding), client)
        iconv.iconv string
      end
      def utf8 string, client=Spreadshiit.client_encoding, to_encoding = 'UTF-8'
        string = string.dup
        key = [to_encoding, client]
        iconv = @@iconvs[key] ||= Iconv.new(build_output_encoding(to_encoding), client)
        iconv.iconv string
